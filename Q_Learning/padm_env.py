...
class CatAndMouseEnv(gym.Env):
    def __init__(self, grid_size=11, goal_coordinates=(11, 11), cat_states= [(3,3), (6, 6), (8, 8)]):
    ...
    #def render(self):
        ...
        #for cat in self.cat_states:
            #hell = pygame.Rect(cat[1] * self.cell_size, cat[0] * self.cell_size, self.cell_size, self.cell_size)
            #pygame.draw.rect(self.screen, (255, 0, 0), hell)

        # Draw the agent:
        #agent = pygame.Rect(self.state[1] * self.cell_size, self.state[0] * self.cell_size, self.cell_size, self.cell_size)
...
def create_env(goal_coordinates, cat_states):
    # Create the environment:
        ...
        for i in range(len(cat_states)):
            env.add_cat_states(cat_states=cat_states[i])
        return env
