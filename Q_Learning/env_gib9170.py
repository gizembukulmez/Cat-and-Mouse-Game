...
class CatAndMouseEnv(gym.Env):
    def __init__(self, grid_size=10):
        ...
        self.goal_coordinates = [np.array([11, 11])]
        self.goal_coordinates = np.array([self.grid_size - 1, self.grid_size - 1])
        self.cat_states = [np.array([3, 3]), np.array([6, 6]), np.array([8, 8])]
        ...
        # Upload voice effect
        self.tom_and_jerry_sound = pygame.mixer.Sound("/Users/afragizembukulmez/Desktop/Principles of Autonomy and Decision Making/Assignment 1/tomandjerry.mp3")
    ...
    def step(self, action):
        ...
        if action == 0 and self.mouse_state[1] < self.grid_size - 1:  # Up
            self.mouse_state[1] += 1
        elif action == 1 and self.mouse_state[1] > 0:  # Down
            self.mouse_state[1] -= 1
        elif action == 2 and self.mouse_state[0] > 0:  # Left
            self.mouse_state[0] -= 1
        elif action == 3 and self.mouse_state[0] < self.grid_size - 1:  # Right
            self.mouse_state[0] += 1

        #if action == 0 and self.mouse_state[0] > 0:  # Yukarı
            #self.mouse_state[0] -= 1
        #elif action == 1 and self.mouse_state[0] < self.grid_size - 1:  # Aşağı
            #self.mouse_state[0] += 1
        #elif action == 2 and self.mouse_state[1] > 0:  # Sol
            #self.mouse_state[1] -= 1
        #elif action == 3 and self.mouse_state[1] < self.grid_size - 1:  # Sağ
            #self.mouse_state[1] += 1
...
if __name__ == "__main__":
    ...
    while running:
        ...
        if done:
            print("Game Over!")
            state = env.reset()  # Reset the environment to start again
            running = False  # Set this to True if you want the game to restart automatically
